...
namespace GastCoCo {
    ...
    public:
        ...
        CBList(std::string infopath, GraphMode graph_mode, bool load_graph_in_order);
        CBList(VertexID Node, EdgeID Edge, std::string path);
        CBList(VertexID Node, EdgeID Edge, std::string path, bool incoming);
        CBList(VertexID Node, EdgeID Edge, std::string path, GraphMode mode);
    ...
    CBList::CBList(std::string infopath, GraphMode graph_mode, bool load_graph_in_order) {
        if (graph_mode != GraphMode::Pull && graph_mode != GraphMode::Push && graph_mode != GraphMode::Mixed) {
            printf("mode error!\n");
            exit(-1);
        }
        ...
        else if (graph_mode == GraphMode::Push) {
        ...
        if (graph_mode == GraphMode::Pull || graph_mode == GraphMode::Mixed)
            ConnectGTChain(this->GTChainIn, this->VertexTableIn);
        if (graph_mode == GraphMode::Push || graph_mode == GraphMode::Mixed)
    ...
    CBList::CBList(VertexID VertexNum, EdgeID EdgeNum, std::string path, GraphMode mode) :writelockOut(VertexNum), writelockIn(VertexNum), vertex_lock(VertexNum) {
