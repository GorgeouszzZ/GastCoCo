...
template <typename T = void> struct [[nodiscard]] generator {
  ...
  struct promise_type {
    ...
    auto initial_suspend() { 
      // std::cout << "coro is initialing\n";
      return std::suspend_always{}; } // return std::suspend_never{};
...
template<> struct generator<void>::promise_type {
  ...
  auto initial_suspend() { 
    // std::cout << "coro is initialing\n";
    return std::suspend_never{}; } //return std::suspend_always{};
...
void SimpleScheduler(std::vector<std::coroutine_handle<>>& tasks)
{
    int finished = 0;
...
//没有赋值即done的工作，小心inital_suspend never
// void SimpleScheduler(std::vector<std::coroutine_handle<>>& tasks)
// {
//     int finished = 0;
//     while(finished < tasks.size())
