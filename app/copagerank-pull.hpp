...
void pagerank(GastCoCo::CBList& graph, int thread_num, int coro_num, int iter)
{
    ...
    // iter *= all_coro_num; 
    vector<double> node_state_old(graph.VertexNum, (double)1);
    vector<double> node_state_new(graph.VertexNum, 1-d);
    uint32_t now_node = 0;
    
    auto SFCresult = SliceForCoro(graph, all_coro_num);
    ...
    for(GastCoCo::IndexType it = 0; it < iter; ++it)
    {
        ...
        for(uint32_t tid=0;tid<thread_num;++tid)
        {
            ...
            {
                // PrTasks[i]=pagerank_oi_hybrid1(G, SFCresult[tid * coro_num + i], SFCresult[tid * coro_num + i+1 ], node_state_old, node_state_new, true).get_handle();
                PrTasks[i]=pagerank_one_iter(graph, SFCresult[tid * coro_num + i], SFCresult[tid * coro_num + i+1 ], node_state_old, node_state_new, true).get_handle();
            }
        ...
        swap(node_state_new, node_state_old);
        #pragma omp parallel for
        for(GastCoCo::VertexID vid=0; vid<graph.VertexNum; ++vid)
            node_state_new[vid] = 1-d;
    }
